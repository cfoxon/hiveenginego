...
func (h HiveEngineRpcNode) getSymbolNFTCount(nftSymbol string) int {
    ...
    qParamsIndex := []ContractQueryParamsIndex{ContractQueryParamsIndex{Index: "_id", Descending: true}}
    ...
    if err := json.Unmarshal(res, &nft); err != nil {
        fmt.Println("Can not unmarshal result for nft count")
        fmt.Println(err)
        fmt.Println(res)
    }
...
func (h HiveEngineRpcNode) GetSymbolAllNft(nftSymbol string) ([]EngineNft, error) {
    ...
    nftSymbolUpper := strings.ToUpper(nftSymbol)
    collectionSize := h.getSymbolNFTCount(nftSymbolUpper)
