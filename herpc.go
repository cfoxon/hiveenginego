...
func (h *HiveEngineRpcNode) rpcExec(endpoint string, query herpcQuery) ([]byte, error) {
    rpcClient := jsonrpc2client.NewClientWithOpts(h.RpcNode.address + endpoint, h.RpcNode.MaxConn, h.RpcNode.MaxBatch)
...
func (h *HiveEngineRpcNode) rpcExecBatchFast(endpoint string, queries []herpcQuery) ([][]byte, error) {
    ...
    var jr2queries jsonrpc2client.RPCRequests
    for i, query := range queries {
        jr2query := &jsonrpc2client.RpcRequest{Method: query.method, JsonRpc: "2.0", Id: i+1, Params: query.params}
        jr2queries = append(jr2queries, jr2query)
    }
